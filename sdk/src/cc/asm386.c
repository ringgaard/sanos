//
//  asm386.c - Tiny C Compiler for Sanos
// 
...
static void masm_expr(TCCState *s1, Operand *op) {
  ...
  for (;;) {
    ...
    // Combine symbols
    if (opr == '+') {
      if (op->e.sym != NULL && op2.e.sym != NULL) goto cannot_relocate;
      if (op->e.sym == NULL && op2.e.sym != NULL) op->e.sym = op2.e.sym;
    } else {
...
static void parse_masm_operand(TCCState *s1, Operand *op) {
  ...
  if (tok >= TOK_ASM_al && tok <= TOK_ASM_db7) {
    ...
    if (tok == '(') {
      next();
      if (tok != TOK_PPNUM) error("illegal fp register");
      p = tokc.cstr->data;
      reg = p[0] - '0';
      next();  
      if ((unsigned) reg >= 8 || p[1] != '\0') error("illegal fp register number");
      op->reg = reg;
      next();
      skip(')');
    }
...
void asm_opcode(TCCState *s1, int opcode) {
  ...
  seg_prefix = 0;
  for (;;) {
    if (tok == ';' || tok == TOK_LINEFEED || tok == '}') break;
  ...
  if (s == 3) {
    ...
    if (s == 3) {
      if ((opcode == TOK_ASM_push || opcode == TOK_ASM_pop) && (ops[0].type & (OP_SEG | OP_IM8S | OP_IM32))) {
        ...
        if (s == 3) error("cannot infer operand size");
      }
    }
  }
...
printf("v=%x instr_type=%x\n", v, pa->instr_type);
  ...
  if (pa->instr_type & OPC_MODRM) {
    ...
    for (i = 0;i < nb_ops; i++) {
      if (op_type[i] & (OP_REG | OP_MMX | OP_SSE | OP_INDIR | OP_FAR)) goto modrm_found;
    }
#ifdef ASM_DEBUG
    error("bad op table");
#endif      
  ...
  try_next:
    ...
    switch (c) {
      ...
      case 'A':
        ...
        if (is_reg_allocated(TREG_EAX) || is_reg_allocated(TREG_EDX)) goto try_next;
        op->is_llong = 1;
        op->reg = TREG_EAX;
      ...
      case 'N':
      case 'M':
        if (!((op->vt->r & (VT_VALMASK | VT_LVAL | VT_SYM)) == VT_CONST)) goto try_next;
        break;
...
void subst_asm_operand(CString *add_str, SValue *sv, int modifier) {
  ...
  if ((r & VT_VALMASK) == VT_CONST) {
    if (!(r & VT_LVAL) && modifier != 'c' && modifier != 'n') cstr_ccat(add_str, '$');
    if (r & VT_SYM) {
