...
err_t tcp_enqueue(struct tcp_pcb *pcb, void *arg, unsigned short len, int flags, int copy, unsigned char *optdata, int optlen)
{
  ...
  if (len == 0 && flags == TCP_ACK) 
  {
    ...
    if (p == NULL) 
    {
      kprintf("tcp_enqueue: (ACK) could not allocate pbuf\n");
      return -EBUF;
    }
    ...
    {
      kprintf("tcp_enqueue: (ACK) no room for TCP header in pbuf.\n");
    ...
    tcphdr->chksum = 0;
    tcphdr->chksum = inet_chksum_pseudo(p, &(pcb->local_ip), &(pcb->dest_ip), IP_PROTO_TCP, p->tot_len);
    
    seg = (struct tcp_seg *) kmalloc(sizeof(struct tcp_seg));
    if (seg == NULL) 
    {
      kprintf("tcp_enqueue: (ACK) could not allocate memory for seg\n");
      stats.tcp.memerr++;
      pbuf_free(p);
      return -ENOMEM;
    }
  ...
  else 
  {
	...
	goto memerr;
      ...
      seg->len = seglen;
      if ((flags & TCP_SYN) || (flags & TCP_FIN)) seg->len++;
	...
	// Options can thus only be sent in non data carrying segments such as SYN|ACK
	memcpy(seg->dataptr, optdata, optlen);
      }

      kprintf("tcp_enqueue: queueing %lu:%lu (0x%x)\n",	ntohl(seg->tcphdr->seqno), ntohl(seg->tcphdr->seqno) + seg->len, flags);
...
err_t tcp_output(struct tcp_pcb *pcb)
{
  ...
  while (seg != NULL &&	ntohl(seg->tcphdr->seqno) - pcb->lastack + seg->len <= wnd) 
  {
    ...
    if (pcb->state != SYN_SENT) 
    {
      ...
      if (pcb->flags & TF_ACK_NEXT) pcb->flags &= ~TF_ACK_NEXT;
    }
...
void tcp_rexmit_seg(struct tcp_pcb *pcb, struct tcp_seg *seg)
{
  ...
  kprintf("tcp_rexmit_seg: skickar %ld:%ld\n", ntohl(seg->tcphdr->seqno), ntohl(seg->tcphdr->seqno) + seg->len);
