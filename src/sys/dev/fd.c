...
static void fd_motor_task(void *arg)
{
  ...
  while (1)
  {
    ...
    while (fdc.dor & 0xF0)
    {
      ...
      {
	if (fddrives[i].motor_status == FD_MOTOR_DELAY && fddrives[i].motor_timeout < get_tick_count())
	{
	  fdc.dor &= ~(0x10 << i);
...
void init_fd()
{
  ...
  fdmotor_task = create_task(fd_motor_task, NULL, PRIORITY_NORMAL);
  mark_thread_ready(fdmotor_task);
}
